...
import { User, useUser } from "@/components/providers/user-provider";
...
export default function Profile({ profile, dayMap }: { profile?: User, dayMap: Map<string, Entry>; }) {
    const { user } = useUser();
                ...
                <div className="w-full flex items-center justify-between gap-5">
                    <div className="flex flex-row items-center gap-5">
                        <Avatar className="size-16 md:size-25 rounded-full">
                            <AvatarImage src={profile?.avatar_url ?? "/avatar.png"} alt={profile?.id} />
                            <AvatarFallback>{(profile?.name ?? profile?.username ?? "User").charAt(0)}</AvatarFallback>
                        </Avatar>
